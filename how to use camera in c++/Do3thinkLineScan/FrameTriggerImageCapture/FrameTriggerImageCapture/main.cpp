...
void test(void* p)
{
    ...
    //设置帧超时
    CheckStatus(status = dvpSetIntValue(h, "FrameTimeout", 0), "dvpSetIntValue FrameTimeout");
    ...
    CheckStatus(status = dvpSetTriggerState(h, true), "dvpSetTriggerState");
    
    CheckStatus(status = dvpGetBoolValueSync(h, "TriggerMode", FrameTrigger), "dvpGetBoolValueSync");
    ...
    CheckStatus(status = dvpSetTriggerSource(h, TRIGGER_SOURCE_LINE3), "dvpSetTriggerSource");
    CheckStatus(status = dvpGetTriggerSource(h, &TriggerSource), "dvpGetTriggerSource");
    
   
    
    printf("%d", TriggerSource);
