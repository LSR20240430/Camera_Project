...
void test(void* p)
{
    ...
    do {
        ...
        //横向翻转使能
        /* bool GetFlipHorizontalState;
         bool SetFlipHorizontalState = true;

         Status = dvpSetFlipHorizontalState(h, SetFlipHorizontalState);
         Status = dvpGetFlipHorizontalState(h, &GetFlipHorizontalState);
         ...
         Status = dvpSetFlipVerticalState(h, SetFlipVerticalState);
         Status = dvpGetFlipVerticalState(h, &GetFlipVerticalState);
        ...
        Status = dvpSetRotateState(h, SetRotateState);
       
        Status = dvpGetRotateState(h, &GetRotateState);
        printf(" GetRotateState: %s\n", GetRotateState ? "true" : "false");
       
        //图像旋转方向的标志，true代表逆时针，false顺时针，头文件注释和软件开发手册都是反的，默认是



       bool GetRotateState1, SetRotateState1 = true;
        Status = dvpSetRotateOpposite(h, SetRotateState1); 
        Status = dvpGetRotateOpposite(h, &GetRotateState1);
       
        printf(" 图像旋转方向标志： %s\n", GetRotateState1 ? "true" : "false");
        
        //设置图像旋转角度，2代表180°，1代表90°，3代表270°
        int RotateAngle = 1;
       
        Status = dvpRotateImage(h, RotateAngle);
