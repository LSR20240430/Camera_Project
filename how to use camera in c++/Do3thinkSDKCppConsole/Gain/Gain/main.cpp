...
void test(void* p)
{
    ...
    do
    {
        ...
        Status = dvpSetAnalogGain(h, SetAnalogGain);
        if (Status != DVP_STATUS_OK) {
            Log_Error("dvpSetAnalogGain", Status);
            break;
        }


           /* Status = dvpGetAnalogGain(h, &AnalogGain);
            if (Status != DVP_STATUS_OK) {
                Log_Error("dvpGetAnalogGain", Status);
                break;
            }*/
            ...
            for (int i = 0; i < 10; i++) {
                ...
                Status = dvpGetFrame(h, &Frame, &pBuffer, 4000);
                if (Status != DVP_STATUS_OK) {
                    Log_Error("dvpGetFrame", Status);
                    break;
                }
                printf("AnalogGain：%lf\r\n", SetAnalogGain);
                printf("Name:%s, Frame:%lld, Timestamp:%lld, %d*%d, %dBytes, Format:%d\r\n",
                    Name,
                    Frame.uFrameID,
                    Frame.uTimestamp,
                    Frame.iWidth,
