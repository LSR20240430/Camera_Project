...
{
    public partial class BasicFunctionRecord : Form
    {
        ...
        public static Boolean m_bRecordFlg;
        public static String m_VideoPath;
        public static uint m_RecHandle; //record handle;
        ...
        public static int _dvpStreamCallback(/*dvpHandle*/uint handle, dvpStreamEvent _event, /*void **/IntPtr pContext, ref dvpFrame refFrame, /*void **/IntPtr pBuffer)
        {
            ...
            if (m_bRecordFlg)
            {
                bool isVideoRecorderValid = false;
                dvpStatus status = DVPCamera.dvpIsVideoRecorderValid(m_RecHandle, ref isVideoRecorderValid);
                Debug.Assert(status == dvpStatus.DVP_STATUS_OK);

                if (isVideoRecorderValid)
                {
                    status = DVPCamera.dvpVideoRecordWrite(m_RecHandle, ref refFrame, pBuffer);
                    Debug.Assert(status == dvpStatus.DVP_STATUS_OK);
                }
                else
                {
                    status = DVPCamera.dvpStartVideoRecord(m_VideoPath, 0, 0, 100, ref m_RecHandle);
                    Debug.Assert(status == dvpStatus.DVP_STATUS_OK);
                }
            }
        ...
        private void BUTTON_RECORD_Click(object sender, EventArgs e)
        {
            ...
            { 
                ...
                sfd.Filter = "AVI Files (*.avi)|*.avi|WMV Files (*.wmv)|*.wmv";
                if (sfd.ShowDialog() == DialogResult.OK)
                {
                    m_VideoPath = sfd.FileName;

                    status = DVPCamera.dvpStartVideoRecord(m_VideoPath, 0, 0, 100, ref m_RecHandle);
                    Debug.Assert(status == dvpStatus.DVP_STATUS_OK);

                    status = DVPCamera.dvpIsVideoRecorderValid(m_RecHandle, ref recValid);
                    Debug.Assert(status == dvpStatus.DVP_STATUS_OK && recValid == true);
            ...
            else
            {
                status = DVPCamera.dvpIsVideoRecorderValid(m_RecHandle, ref recValid);
                Debug.Assert(status == dvpStatus.DVP_STATUS_OK && recValid == true);
                status = DVPCamera.dvpStopVideoRecord(m_RecHandle);
                
                m_bRecordFlg = false;
                BUTTON_RECORD.Text = "Start";

                System.Diagnostics.Process.Start(m_VideoPath);
