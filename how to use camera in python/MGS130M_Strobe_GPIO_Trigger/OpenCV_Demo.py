...
def SetStrobe(camera):
    camera.SetLineMode(Line.LINE_2, LineMode.LINE_MODE_OUTPUT)
    camera.SetLineSource(Line.LINE_2, LineSource.OUTPUT_SOURCE_STROBE)
    camera.StrobeOutputType = StrobeOutputType.STROBE_OUT_HIGH

    # Take effect when sensor is exposure time
    camera.StrobeDriver = StrobeDriver.FRAME_DURATION

    # Driver by a timer
    # camera.StrobeDriver = StrobeDriver.TIMER_LOGIC
    # camera.StrobeDuration = 15000


def SetGPIO(camera):
    camera.SetLineMode(Line.LINE_3, LineMode.LINE_MODE_OUTPUT)
    camera.SetLineSource(Line.LINE_3, LineSource.OUTPUT_SOURCE_NORMAL)
    camera.SetLineInverter(Line.LINE_3, True)
...
def main():
    ...
    while (cv2.waitKey(1) != 27):                           #Press the ESC key to exit the loop
        try:
            print(camera.FrameCount.uFrameOK)                       #Print frame information
