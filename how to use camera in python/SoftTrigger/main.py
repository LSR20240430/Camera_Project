...
def set_SoftTrigger(camera):
    camera.TriggerState = True
    camera.TriggerInputType = TriggerInputType.TRIGGER_POS_EDGE
    camera.TriggerSource = TriggerSource.TRIGGER_SOURCE_SOFTWARE

def fire_software_trigger(camera):
    """执行软件触发"""
    try:
        camera.TriggerFire = 1 # 发送软触发信号。必须要赋值整型类型；1表示软触发1帧
        print("已发送软触发信号")
        return True
    except dvpException as e:
        print(f"触发失败: {e.Status}")
        return False
